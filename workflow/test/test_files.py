...
def test_derived(source):
    dd = workflow.conf.rcParams['data dir']
    assert(source.url('0601') == "https://www.url.gov/NHDPlus_H_0601_GDB.zip")
    assert(source.file_name('0601') == os.path.join(dd,'hydrography/NHDPlus_H_0601_GDB/NHDFlowline.shp'))
    assert(source.download('0601') == os.path.join(dd,'hydrography/zips/NHDPlus_H_0601_GDB.zip'))


def test_download():
    fm = workflow.files.NHDFileManager()
    src = fm.names
    print(src.download('0601'))
    print(src.url('0601'))
    workflow.files._download(src.url('0601'),src.download('0601'))
    workflow.files._unzip(src.download('0601'), src.folder_name('0601'))
