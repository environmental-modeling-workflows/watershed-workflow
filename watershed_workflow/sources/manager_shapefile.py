...
class ManagerShapefile(ManagerShapes):
    ...
    def __init__(self, filename: str, id_name: Optional[str] = None):
        ...
        native_crs_in = watershed_workflow.crs.from_string(info['crs'])
        ...
        native_resolution = min(width, height) / 1000.0
        ...
        super().__init__(name, source, native_crs_in, native_resolution, native_id_field)
    ...
    def _getShapesByGeometry(self, geometry_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        df = gpd.read_file(self._filename, bbox=union_geometry.bounds)
        return df
    ...
    def _getShapesByID(self, ids: List[str]) -> gpd.GeoDataFrame:
        ...
        if self._id_name is not None:
            ...
            if self._id_name not in df.columns:
                raise ValueError(f"ID field '{self._id_name}' not found in shapefile columns: {list(df.columns)}")
                
            id_column = df[self._id_name]
            if len(id_column) > 0:
                target_type = type(id_column.iloc[0])
                try:
                    converted_ids = [target_type(id_val) for id_val in ids]
                    df = df[df[self._id_name].isin(converted_ids)]
                except (ValueError, TypeError) as e:
                    raise ValueError(f"Cannot convert IDs {ids} to type {target_type} for field '{self._id_name}': {e}")
        else:
            ...
            if len(valid_indices) == 1:
                ...
                df = gpd.read_file(self._filename, rows=slice(index, index + 1))
            else:
    ...
    def _addStandardNames(self, df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        if self._id_name is not None and self._id_name in df.columns:
            df[names.ID] = df[self._id_name]
        else:
