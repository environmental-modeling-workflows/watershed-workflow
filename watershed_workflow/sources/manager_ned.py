...
class FileManagerNED:
    ...
    def __init__(self, resolution='1/3 arc-second', file_format='GeoTIFF'):
        ...
        self.names = watershed_workflow.sources.names.Names(
            self.name, 'dem', None, 'USGS_NED_%s_n{:02}_w{:03}.%s' %
            (self.short_res, file_extension), self.short_res + "_raw")
    ...
    def request(self, bounds):
        ...
        rest_bounds = ','.join(str(b)
                               for b in bounds)  #[bounds[1], bounds[0], bounds[3], bounds[2]])
        try:
    ...
    def download(self, bounds, force=False):
        ...
        filenames = [
            self.names.file_name(j + 1, -i) for j in range(south, north) for i in range(west, east)
        ]
        logging.info('  Need:')
        ...
        if (any(not os.path.exists(f) for f in filenames) or force):
            ...
            for r in request['items']:
                ...
                if not os.path.exists(filename) or force:
                    ...
                    downloadfile = os.path.join(self.names.raw_folder_name(north, west),
                                                downloadfilename)
                    ...
                    if downloadfile.endswith('.ZIP') or downloadfile.endswith('.zip'):
                        ...
                        if os.path.isdir(os.path.join(work_dir, unzip_filename)):
                            img_files = [
                                ...
                                for f in os.listdir(os.path.join(work_dir, unzip_filename))
                        ...
                        if len(img_files) == 0:
                            ...
                            logging.debug("  Found '{}'".format(os.path.join(
                                work_dir, img_files[0])))
                ...
                if not os.path.exists(filename):
                    raise RuntimeError(
                        '{}: Cannot find or download file for source target "{}"'.format(
                            self.name, filename))
                else:
            ...
            if len(filenames) != 0:
                logging.warn(
                    'Potentially missing tiles in the DEM covering bounds: {}'.format(bounds))
                logging.warn(
