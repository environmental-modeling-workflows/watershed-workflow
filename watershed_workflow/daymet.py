...
def collectDaymet(bounds, crs, start, end, vars=None, force=False, buffer=0.01):
    ...
    daymet_obj = watershed_workflow.sources.manager_daymet.FileManagerDaymet()

    for year in range(start.year, end.year + 1):
        for var in vars:
            fname, feather_bounds = daymet_obj.get_meteorology(var,
                                                               year,
                                                               bounds,
                                                               crs,
...
def reproj_Daymet(x, y, raw, dst_crs, resolution=None):
    ...
    if unit == 'km':
        dx = dy = 1.0  # km
        transform = (x.min() / 1000 - dx/2, dx, 0.0, y.max() / 1000 + dy/2, 0.0, -dy
                     )  # accepted format(xmin, dx, 0, ymax, 0, -dy)
        ...
        raise RuntimeError(
            f'Daymet CRS unit: {unit} is not recognized! Supported units are m or km.')
    ...
    for var in var_list:
        ...
        idat = raw[var]
        dst_profile, dst_raster = watershed_workflow.warp.raster(src_profile=daymet_profile,
                                                                 src_array=idat,
                                                                 dst_crs=dst_crs,
    ...
    new_extent = rasterio.transform.array_bounds(dst_profile['height'], dst_profile['width'],
                                                 dst_profile['transform'])  # (x0, y0, x1, y1)
...
def smoothRaw(raw, smooth_filter=True, nyears=None):
    ...
    for ivar in var_list:
        ...
        if smooth_filter:
            ...
            poly_order = 2
            logging.info(
                f"smoothing {ivar} using savgol filter, window = {window} d, poly order = {poly_order}"
            )
