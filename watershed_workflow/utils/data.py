...
__all__ = [
    'ValidTime',
    'convertTimesToCFTime',
    'convertTimesToCFTimeNoleap',
    'createNoleapMask',
    'computeMode',
    'filterLeapDay',
    'interpolate',
    'interpolateToRegular',
    'computeAverageYear',
    'smoothTimeSeries',
    'smooth2D',
    'interpolateValues',
...
def smoothTimeSeries_Array(data: np.ndarray,
    ...
    preserve_nans : bool, optional
        If True, NaN positions in the input are restored as NaN in the output
        after smoothing. If False, NaNs within a partially-valid slice are
        imputed by linear interpolation before filtering and the smoothed
        values are kept. Slices that are entirely NaN are always left as NaN.
        Default is False.
    **kwargs : dict
    ...
    if method == 'savgol':
        ...
        # Fast path: no NaNs at all
        if not np.any(np.isnan(data)):
            return scipy.signal.savgol_filter(data, window_length, polyorder, axis=axis, mode=mode)

        result = data.copy()
        x = np.arange(data.shape[axis])
        for idx in np.ndindex(*[s for i, s in enumerate(data.shape) if i != axis]):
            full_idx = idx[:axis] + (slice(None),) + idx[axis:]
            col = data[full_idx]
            nan_mask = np.isnan(col)
            if nan_mask.all():
                # Leave entirely-NaN slices as NaN
                continue
            if nan_mask.any():
                # Impute NaNs by linear interpolation before filtering
                col = np.interp(x, x[~nan_mask], col[~nan_mask])
            smoothed = scipy.signal.savgol_filter(col, window_length, polyorder, axis=0, mode=mode)
            if preserve_nans:
                smoothed[nan_mask] = np.nan
            result[full_idx] = smoothed
        return result
...
def interpolateValues(points: np.ndarray,
    ...
    if points_crs is not None and array_crs is not None:
        x, y = watershed_workflow.utils.warp.warpXY(points[:, 0], points[:, 1], points_crs, array_crs)
    else:
