...
def warpXY(x: np.ndarray, y: np.ndarray, old_crs: CRS, new_crs: CRS) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def warpPoints(array: np.ndarray, old_crs: CRS, new_crs: CRS) -> np.ndarray:
    """
    ...
    """
    x, y = warpXY(array[:, 0], array[:, 1], old_crs, new_crs)
    return np.array([x, y]).transpose()


def warpBounds(bounds: Tuple[float, float, float, float], old_crs: CRS,
    ...
    """
    return warpShply(shapely.geometry.box(*bounds), old_crs, new_crs).bounds


def warpShply(shp: shapely.geometry.base.BaseGeometry, old_crs: CRS,
...
def warpShplys(shps: list, old_crs: CRS, new_crs: CRS) -> list:
    """
