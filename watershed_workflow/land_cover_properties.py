...
def computeCrosswalkCorrelation(modis_lc : xarray.DataArray,
    ...
    if plot:
        ...
        ax.set_yticklabels([f'{id} : {nlcd_colors[id][0]} ({nlcd_counts[i]} px)' for (i,id) in enumerate(unique_nlcd)])
...
def computeCrosswalk(modis_lc : xarray.DataArray,
    ...
    """
    unique_nlcd, unique_modis, corr, counts = computeCrosswalkCorrelation(modis_lc, nlcd_lc, **kwargs)
    crosswalk = dict()
    ...
    for i, nlcd in enumerate(unique_nlcd):
        if method == 'maximal area':
            ...
            crosswalk[nlcd] = [(unique_modis[j], corr[i, j])
                               for j in range(len(unique_modis)) if corr[i, j] > 0]
        else:
            raise ValueError(
...
def applyCrosswalk(crosswalk : Dict[int, List[Tuple[int, float]]],
    ...
    """Given a crosswalk from unique NLCD IDs to MODIS, computes time
    series data for each NLCD ID based on the MODIS LAI data.
    """
    ...
    for nlcd_id in unique_nlcd:
        ...
        nlcd_lai[_nlcdName(nlcd_id)] = sum(part[1] * modis_lai[_modisName(part[0])].to_numpy() for part in cw)
