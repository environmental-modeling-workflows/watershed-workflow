...
def test_simplify():
    ...
    for i in range(100):
        ...
        mygood = ((len(ls_s.coords) == 2)
                  and watershed_workflow.utils.utils.isClose(ls_s.coords[0], ls.coords[0], 1.e-10)
                  and watershed_workflow.utils.utils.isClose(ls_s.coords[-1], ls.coords[-1], 1.e-10))
        good.append(mygood)
...
def test_snap():
    ...
    assert (watershed_workflow.utils.utils.isClose(l2.coords[1], ls.coords[0], 1.e-8))
...
def test_snap2():
    ...
    assert (watershed_workflow.utils.utils.isClose(ls.coords[0], l.coords[0], 1.e-8))
