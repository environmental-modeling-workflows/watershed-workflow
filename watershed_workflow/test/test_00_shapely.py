...
def test_intersection_intersects():
    ...
    shp = shapely.geometry.Polygon([(1.03425, 0.0013), (0.0035, 1.03523), (-1.09824, 0.0033),
                                    (0.0012, -1.04856)])
...
def test_simplify():
    ...
    for i in range(100):
        ...
        mygood = ((len(ls_s.coords) == 2)
                  and watershed_workflow.utils.geometry.isClose(ls_s.coords[0], ls.coords[0], 1.e-10)
                  and watershed_workflow.utils.geometry.isClose(ls_s.coords[-1], ls.coords[-1], 1.e-10))
        good.append(mygood)
...
def test_snap():
    ...
    assert (watershed_workflow.utils.geometry.isClose(l2.coords[1], ls.coords[0], 1.e-8))
...
def test_snap2():
    ...
    assert (watershed_workflow.utils.geometry.isClose(ls.coords[0], l.coords[0], 1.e-8))
