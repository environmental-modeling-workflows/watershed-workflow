...
def test_river_tree_properties(sources_download):
    ...
    _, reaches = watershed_workflow.get_reaches(nhd, '060102020103', cc.exterior().bounds, crs, crs, properties=True)

    rivers = watershed_workflow.construct_rivers(cc, reaches, method='hydroseq', filter=True)
    assert(len(rivers) == 1)
...
def test_river_tree_properties_prune(sources_download):
    ...
    rivers = watershed_workflow.construct_rivers(cc, reaches, method='hydroseq', filter=True, prune_by_area_fraction=0.03)
    assert(len(rivers) == 1)
    assert(rivers[0].is_consistent())
...
def test_river_tree_geometry(sources):
    ...
    nhd = sources['HUC']
    _, cc = watershed_workflow.get_split_form_hucs(nhd, '060102020103', 12, crs)
    _, reaches = watershed_workflow.get_reaches(nhd, '060102020103', cc.exterior().bounds, crs, crs, properties=False)

    rivers = watershed_workflow.construct_rivers(cc, reaches, filter=True)
    assert(len(rivers) == 1)
    assert(rivers[0].is_consistent())
