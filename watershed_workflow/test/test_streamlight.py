...
def sl_data():
    ...
    data_ts = input_data_test_streamlight_df = pd.read_csv(
        os.path.join(path_data_test, 'data_test_streamlight_time_series.csv'))

    data_params = input_data_test_streamlight_df = pd.read_csv(
        os.path.join(path_data_test, 'data_test_streamlight_parameters.csv'))
...
def test_solar_dec(sl_data, relative_tolerance=1e-05, absolute_tolerance=1e-08, plot_scatter=False):
    ...
    assert np.allclose(sl.solar_angles[var_name_ww],
                       data_ts[var_name_test].values,
    ...
    if plot_scatter:
        ...
        plot_comparison(sl.solar_angles[var_name_ww], data_ts[var_name_test].values, var_name_ww,
                        ax)
...
def test_solar_altitude(sl_data,
...
def test_solar_azimuth(sl_data,
                       relative_tolerance=1e-05,
                       absolute_tolerance=1e-08,
                       plot_scatter=False):
...
def test_total_trans_PAR_ppfd(sl_data,
                              relative_tolerance=1e-05,
                              absolute_tolerance=1e-08,
                              plot_scatter=False):
    ...
    assert np.allclose(sl.energy_response[var_name_ww],
                       data_ts[var_name_test].values,
                       rtol=relative_tolerance,
                       atol=absolute_tolerance,
                       equal_nan=True)
    ...
    if plot_scatter:
        fig, ax = plt.subplots(figsize=(5, 5))
        plot_comparison(sl.energy_response[var_name_ww], data_ts[var_name_test].values, var_name_ww,
                        ax)
...
def test_fraction_shade_veg(sl_data,
                            relative_tolerance=1e-05,
                            absolute_tolerance=1e-08,
                            plot_scatter=False):
    ...
    assert np.allclose(sl.shading_response[var_name_ww],
                       data_ts[var_name_test].values,
                       rtol=relative_tolerance,
                       atol=absolute_tolerance,
                       equal_nan=True)
    ...
    if plot_scatter:
        fig, ax = plt.subplots(figsize=(5, 5))
        plot_comparison(sl.shading_response[var_name_ww], data_ts[var_name_test].values,
                        var_name_ww, ax)
...
def test_fraction_shade_bank(sl_data,
                             relative_tolerance=1e-05,
                             absolute_tolerance=1e-08,
                             plot_scatter=False):
...
def test_energy_total_surface_PAR_ppfd(sl_data,
                                       relative_tolerance=1e-05,
                                       absolute_tolerance=1e-08,
                                       plot_scatter=False):
