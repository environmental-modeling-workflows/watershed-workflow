...
def check0(hucs, rivers):
    ...
    for tree in rivers:
        assert tree.isConsistent()
    assert watershed_workflow.utils.geometry.isClose(riverlist[0],
                                            shapely.geometry.LineString([(5, 0), (10, 0)]),
                                            1.e-3)

    # exact at the endpoint
    assert any(watershed_workflow.utils.geometry.isClose(riverlist[0].coords[-1], c) for c in poly0.boundary.coords)
...
def check1(hucs, rivers, extra_point=None):
    ...
    if extra_point == 'right':
        assert watershed_workflow.utils.geometry.isClose(riverlist[i],
...
def check2(hucs, rivers, extra_point=None):
    ...
    # - paddlers left
    assert watershed_workflow.utils.geometry.isClose(riverlist[1],
                                            shapely.geometry.LineString([(5,-2.5), (10,0)]))

    # - paddlers right
    assert watershed_workflow.utils.geometry.isClose(riverlist[2],
                                            shapely.geometry.LineString([(5,2.5), (10,0)]))
...
def check3(hucs, rivers, extra_point=None):
    ...
    # - paddlers right
    assert watershed_workflow.utils.geometry.isClose(riverlist[3],
                                            shapely.geometry.LineString([(5,2.5), (10,0)]))
