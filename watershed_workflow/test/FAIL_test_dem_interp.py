...
def dem_and_points():
    ...
    xy = np.array([(0.000001, 0.000001), (.5, .5), (.9999999, .9999999), (1, 1),
                   (1.0000001, 1.0000001), (1.5, 1.5), (1.9999999, 1.9999999), (.5, 1.5), (1.5, .5),
                   ])
...
def test_nearest(dem_and_points):
    ...
    vals = watershed_workflow.values_from_raster(
        xy, watershed_workflow.crs.from_rasterio(dem_profile['crs']), dem, dem_profile, 'nearest')
...
def test_interp(dem_and_points):
    dem, dem_profile, xy = dem_and_points
    vals = watershed_workflow.values_from_raster(
        xy, watershed_workflow.crs.from_rasterio(dem_profile['crs']), dem, dem_profile,
        'piecewise bilinear')
