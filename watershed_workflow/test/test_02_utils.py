...
def test_close():
    ...
    assert (watershed_workflow.utils.geometry.isClose(p0, p1))

    p1 = shapely.geometry.Point((0., 0.000001))
    assert (not watershed_workflow.utils.geometry.isClose(p0, p1))

    p1 = shapely.geometry.Point((0., 1.e-8))
    assert (watershed_workflow.utils.geometry.isClose(p0, p1))

    # point and tuples
    assert (watershed_workflow.utils.geometry.isClose(p0, (0, 1.e-8)))
    assert (watershed_workflow.utils.geometry.isClose((0, 0), (0, 1.e-8)))

    # fails
    assert (not watershed_workflow.utils.geometry.isClose((0, 0, 0), (0, 0)))
    ...
    assert (watershed_workflow.utils.geometry.isClose(l0, l1, 0.01))
    assert (not watershed_workflow.utils.geometry.isClose(l0, l1, 0.0001))
    assert (not watershed_workflow.utils.geometry.isClose(l0, l2, 100))
    ...
    assert (watershed_workflow.utils.geometry.isClose(p0, p1, 0.01))
    assert (not watershed_workflow.utils.geometry.isClose(p0, p1, 0.0001))
    assert (watershed_workflow.utils.geometry.isClose(p0, p2, 0.01))
    assert (not watershed_workflow.utils.geometry.isClose(p0, p2, 0.0001))
...
def test_contains():
    ...
    for i in range(100):
        ...
        lines, _ = watershed_workflow.utils.geometry.cut(line, shp.boundary)

        contains.append(watershed_workflow.utils.geometry.contains(shp, lines[0]))
...
def test_cut_point_not_there():
    ...
    lines, _ = watershed_workflow.utils.geometry.cut(line, cut)
    assert (len(lines) == 2)
    l1 = lines[0]
    l2 = lines[1]
    print(l1.coords[:])
...
def test_cut_first_point():
    ...
    assert (watershed_workflow.utils.geometry.isClose(lines[0], line))
...
#     assert (len(lines) == 1)
#     print(list(lines[0].coords))
#     assert (watershed_workflow.utils.geometry.isClose(
...
def test_cut_two_ways():
    ...
    l1_segs, l2_segs = watershed_workflow.utils.geometry.cut(line1, line2)
...
def test_raises():
    ...
    l1, c1 = watershed_workflow.utils.geometry.cut(line, cut)
    assert watershed_workflow.utils.geometry.isClose(l1[0], line)
    assert watershed_workflow.utils.geometry.isClose(c1[0], cut)
...
def test_intersect_point_to_linestring():
    ...
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(0, 0), P(0, 0), P(0, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (0, 0)))

    # test on the last point
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(0, 1), P(0, 0), P(0, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (0, 1)))

    # test on the line
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(0, 0), P(0, -1), P(0, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (0, 0)))

    # test x-perp
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(1, 0), P(0, -1), P(0, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (0, 0)))

    # test diagonal perp
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(1, -1), P(-1, -1), P(1, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (0, 0)))

    # test colinear but negative
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(-2, -2), P(-1, -1), P(1, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (-1, -1)))
    ...
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(-3.3, -2.1), P(-1, -1), P(1, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (-1, -1)))

    # test end but close
    p0 = watershed_workflow.utils.geometry.intersectPointToSegment(P(-.9, -1.1), P(-1, -1), P(1, 1))
    assert (watershed_workflow.utils.geometry.isClose(p0, (-1, -1)))
...
def test_neighborhood():
    ...
    assert (not watershed_workflow.utils.geometry.inNeighborhood(p1, p2))
    ...
    assert (watershed_workflow.utils.geometry.inNeighborhood(p1, p2))
    ...
    p2 = shapely.geometry.LineString([(1.01, 1), (2, 1)])
    assert (not watershed_workflow.utils.geometry.inNeighborhood(p1, p2, 1.e-3))
    ...
    assert (watershed_workflow.utils.geometry.inNeighborhood(p3, p4, 0.))
...
def test_generate_points():
    ...
    with pytest.raises(TypeError):
        for r in watershed_workflow.utils.geometry.generateRings(shapely.geometry.mapping(p)):
            pass
    c1 = list(watershed_workflow.utils.geometry.generateCoords(shapely.geometry.mapping(p)))
    assert (1 == len(c1))
...
def test_generate_lines():
    ...
    for r in watershed_workflow.utils.geometry.generateRings(shapely.geometry.mapping(l1)):
        assert (np.allclose(l1a, np.array(r)))

    for c1, c2 in zip(l1a, watershed_workflow.utils.geometry.generateCoords(shapely.geometry.mapping(l1))):
        assert (watershed_workflow.utils.geometry.isClose(tuple(c1), c2))
...
def test_generate_multilines():
    ...
    ringlist = list(watershed_workflow.utils.geometry.generateRings(shapely.geometry.mapping(ml)))
    assert (np.allclose(l1a, np.array(ringlist[0])))
    assert (np.allclose(l2a, np.array(ringlist[1])))

    coordlist = np.array(
        list(watershed_workflow.utils.geometry.generateCoords(shapely.geometry.mapping(ml))))
...
def test_generate_polygons():
    ...
    for r in watershed_workflow.utils.geometry.generateRings(shapely.geometry.mapping(poly1)):
        assert (np.allclose(poly1a, np.array(r)))

    for c1, c2 in zip(poly1a,
                      watershed_workflow.utils.geometry.generateCoords(shapely.geometry.mapping(poly1))):
        assert (watershed_workflow.utils.geometry.isClose(tuple(c1), c2))
...
def test_generate_multipolygons():
    ...
    ringlist = list(watershed_workflow.utils.geometry.generateRings(shapely.geometry.mapping(mpoly)))
    assert (np.allclose(poly1a, np.array(ringlist[0])))
    assert (np.allclose(poly2a, np.array(ringlist[1])))

    coordlist = np.array(
        list(watershed_workflow.utils.geometry.generateCoords(shapely.geometry.mapping(mpoly))))
...
def test_angle():
    ...
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v2) - 90) < 1.e-10

    v3 = shapely.geometry.LineString([(1,1), (0,0)])
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v3) - .75 * 180) < 1.e-10
    
    v4 = shapely.geometry.LineString([(1,0), (0,0)])
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v4) - 180) < 1.e-10

    v5 = shapely.geometry.LineString([(1,-1), (0,0)])
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v5) - 5. * 180 / 4.) < 1.e-10

    v6 = shapely.geometry.LineString([(0,-1), (0,0)])
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v6) - 6 * 180 / 4.) < 1.e-10


    v7 = shapely.geometry.LineString([(-1,1.e-10), (0,0)])
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v7) - 0.) < 1.e-8

    v8 = shapely.geometry.LineString([(-1,-1.e-10), (0,0)])
    assert abs(watershed_workflow.utils.geometry.computeAngle(v1,v8) - 360) < 1.e-8
...
def test_project():
    ...
    vec2 = watershed_workflow.utils.geometry.projectVectorAtAngle(v1,90,1)
    v2 = shapely.geometry.LineString([np.array(v1.coords[0]) + np.array(vec2), v1.coords[0]])
    assert watershed_workflow.utils.geometry.isClose(v2_gold, v2)

    v3_gold = shapely.geometry.LineString([(1,1), (0,0)])
    vec3 = watershed_workflow.utils.geometry.projectVectorAtAngle(v1, .75*180, np.sqrt(2))
    v3 = shapely.geometry.LineString([np.array(v1.coords[0]) + np.array(vec3), v1.coords[0]])
    assert watershed_workflow.utils.geometry.isClose(v3_gold, v3)
    

    vec4 = watershed_workflow.utils.geometry.projectVectorAtAngle(v1, .75*180, 4)
