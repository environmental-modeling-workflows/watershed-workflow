...
def testInternalAngleLen3Null():
    ...
    assert count == 0
    assert watershed_workflow.utils.geometry.isClose(ls, ls2)
...
def testInternalAngleLen3RemovePoint():
    ...
    assert watershed_workflow.utils.geometry.isClose(ls.coords[0], ls2.coords[0])
    assert watershed_workflow.utils.geometry.isClose(ls.coords[-1], ls2.coords[-1])
...
def testJunctionAngles():
    ...
    linestrings = [watershed_workflow.utils.geometry.reverseLineString(river.linestring),] \
...
def testJunctionIsBadAngle():
    ...
    assert watershed_workflow.utils.geometry.isClose(river_orig.linestring, river.linestring)
...
def testHUCOutletIsBad():
    ...
    # river is unchanged
    assert watershed_workflow.utils.geometry.isClose(river.linestring, river_copy.linestring)
...
def testHUCIsBad_NoOp(two_boxes):
    ...
    assert count == 0
    assert all(watershed_workflow.utils.geometry.isClose(p1, p2) for p1,p2 in zip(tb.polygons(), tb_copy.polygons()))
