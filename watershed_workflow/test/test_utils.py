...
def test_cut_nearly_first_point():
    ...
    assert(len(lines) == 1)
    print(list(lines[0].coords))
    assert(watershed_workflow.utils.close(lines[0], shapely.geometry.LineString([(0.001,0), (0.5, 0), (1,0)])))
...
def test_cut_nearly_last_point():
    ...
    assert(len(lines) == 1)
    print(list(lines[0].coords))
    assert(watershed_workflow.utils.close(lines[0], shapely.geometry.LineString([(0.,0), (0.5, 0), (0.9999,0)])))
...
def test_perp():
    ...
    p = (0.5,0)
    p3 = watershed_workflow.utils.find_perp(l1, p)
...
def test_merge0():
    ...
    new_ml = watershed_workflow.utils.merge(ml1,ml2)
    assert(len(new_ml) == 2)
    assert(watershed_workflow.utils.close(new_ml[0], l1))
    assert(watershed_workflow.utils.close(new_ml[1], l2))
...
def test_merge1():
    ...
    assert(len(new_ml) == 3)
    assert(watershed_workflow.utils.close(new_ml[0], l1))
    assert(watershed_workflow.utils.close(new_ml[1], shapely.geometry.LineString([(2.5,2.5),(3,3),(3.5,3.5)])))
    assert(watershed_workflow.utils.close(new_ml[2], l4))
...
def test_merge1():
    ...
    new_ml = watershed_workflow.utils.merge(ml1,ml2)
    assert(len(new_ml) == 4)
    assert(watershed_workflow.utils.close(new_ml[0], l3))
    assert(watershed_workflow.utils.close(new_ml[1], shapely.geometry.LineString([(1.5,1.5),(2,2),(2.5,2.5)])))
    assert(watershed_workflow.utils.close(new_ml[2], shapely.geometry.LineString([(2.5,2.5),(3,3),(3.5,3.5)])))
    assert(watershed_workflow.utils.close(new_ml[3], l5))
...
def test_generate_multilines():
    ...
    coordlist = np.array(list(watershed_workflow.utils.generate_coords(shapely.geometry.mapping(ml))))
...
def test_generate_polygons():
    ...
    for c1,c2 in zip(poly1a, watershed_workflow.utils.generate_coords(shapely.geometry.mapping(poly1))):
        assert(watershed_workflow.utils.close(tuple(c1), c2))
...
def test_generate_multipolygons():
    ...
    coordlist = np.array(list(watershed_workflow.utils.generate_coords(shapely.geometry.mapping(mpoly))))
