    ...
    for i, row in enumerate(intersections):
        ...
        for j, entry in enumerate(row):
            ...
            if i <= j:
                ...
                watershed_workflow.utils.geometry.isClose(entry.coords[0], (10, -5))
                watershed_workflow.utils.geometry.isClose(entry.coords[1], (10, 5))
...
def test_hucs(two_boxes):
    ...
    assert (watershed_workflow.utils.geometry.isClose(two_boxes.geometry[0], p0))

    p1 = tb.computePolygon(1)
    assert (len(p1.boundary.coords) == 5)
    assert (watershed_workflow.utils.geometry.isClose(two_boxes.geometry[1], p1))
    ...
    assert (watershed_workflow.utils.geometry.isClose(poly, p3))
    ...
    #     assert(watershed_workflow.utils.geometry.isClose(b1,b2))
...
def test_hucs_triple():
    ...
    assert (watershed_workflow.utils.geometry.isClose(bound1,
    ...
    assert (watershed_workflow.utils.geometry.isClose(spine1, shapely.geometry.LineString([(10, 5),
                                                                                (10, -5)])))

    spine2 = hucs.linestrings[intersections[1]]
    assert (watershed_workflow.utils.geometry.isClose(spine2, shapely.geometry.LineString([(0, 5), (10, 5)])))

    spine3 = hucs.linestrings[intersections[2]]
    assert (watershed_workflow.utils.geometry.isClose(spine3, shapely.geometry.LineString([(10, 5), (20, 5)])))
