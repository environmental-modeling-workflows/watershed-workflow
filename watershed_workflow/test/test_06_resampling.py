...
def test_distance_functor():
    ...
    functor = watershed_workflow.hydro.resampling.TargetLengthByDistance((1,5,2,10),shp)
...
def test_uniform_smaller():
    ...
    coords_out = watershed_workflow.hydro.resampling._resampleLineStringUniform(ls_in, 2.)
    ls_out = shapely.geometry.LineString(coords_out)
    ls_test = shapely.geometry.LineString([(i,0) for i in range(0,21,2)])
    assert watershed_workflow.utils.geometry.isClose(ls_out, ls_test, 1.e-10)
...
def test_smaller():
    ls_in = shapely.geometry.LineString([ (i,0) for i in range(0,21,10)])
    ls_out = watershed_workflow.hydro.resampling.resampleLineStringUniform(ls_in, 2., keep_points=True)
    ls_test = shapely.geometry.LineString([(i,0) for i in range(0,21,2)])
    assert watershed_workflow.utils.geometry.isClose(ls_out, ls_test, 1.e-10)
...
def test_length_func():
    shp = shapely.geometry.Point((0,0))
    strat = watershed_workflow.hydro.resampling.TargetLengthByDistance((2, 4, 10, 8), shp)
    assert strat((1,0)) == 4
    assert strat((2,0)) == 4
    assert strat((6,0)) == 6
    assert strat((10,0)) == 8
...
def test_nonuniform_smaller():
    ...
    strat = watershed_workflow.hydro.resampling.TargetLengthByDistance((4, 4, 10, 8), shp)
    coords_out = watershed_workflow.hydro.resampling._resampleLineStringNonuniform(ls_in, strat)
    ls_out = shapely.geometry.LineString(coords_out)
    ls_test = shapely.geometry.LineString([(0,0), (4,0), (10,0), (18,0)])
    assert watershed_workflow.utils.geometry.isClose(ls_out, ls_test, 1.e-6)
...
def test_nonuniform_no_keep_corner():
    ...
    print(ss[1:] - ss[:-1])
    assert watershed_workflow.utils.geometry.isClose(ls_out.coords[0], ls_in.coords[0])
    assert watershed_workflow.utils.geometry.isClose(ls_out.coords[-1], ls_in.coords[-1])
...
def test_nonuniform_keep_corner():
    ...
    strat = watershed_workflow.hydro.resampling.TargetLengthByDistance((1, 5, 11, 10), shp)
    ls_out = watershed_workflow.hydro.resampling.resampleLineStringNonuniform(ls_in, strat, keep_points=True)
...
def test_resampling(watershed_poly2, watershed_reaches2):
    watershed = watershed_workflow.hydro.watershed.Watershed(watershed_poly2)
    rivers = watershed_workflow.hydro.river.createRivers(watershed_reaches2, method='geometry')

    watershed_workflow.hydro.resampling.resampleWatershed(watershed, (0,25,10,50), 
                                shapely.ops.unary_union([r.to_mls() for r in rivers]))

    watershed_workflow.hydro.resampling.resampleRivers(rivers, 14)
