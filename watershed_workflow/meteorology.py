...
def computeTypicalYear(dat: xr.Dataset,
    ...
    dat = dat[0:len(dat[time_dim]) // 365 * 365]
    ...
    dout = watershed_workflow.data.computeAverageYear(din, din[time_dim][0].year - repeat_nyears, repeat_nyears, time_dim)

    # find the median precip year and insert this into dout
    # -- create a year-block based on the initial day
    block = xr.DataArray(
        np.arange(len(precip_raw.time)) // 365,
        dims=time_dim,
        name="year_block"
    )
    ...
    typical_precip = precip[median_i * 365:(median_i+1)*365]
    typical_precip[time_dim] = dout[time_dim]

    # repeat nyears times
    tiled = xr.concat([typical_precip] * nyears, dim="repeat")
    tiled = tiled.stack(time=("repeat", time_dim)).reset_index(time_dim)
    tiled[time_dim] = dout[time_dim]

    # allocate precip
    dout['precipitation rain [m s^-1]'], dout['precipitation snow [m SWE s^-1]'] = \
        allocatePrecipitation(typical_precip, dout['air temperature [K]'], transition_temperature)
