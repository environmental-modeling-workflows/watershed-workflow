...
__all__ = [
    ...
    'combineSiblings',
    'createRivers',
    'determineOutletToReachMap',
    'accumulateCatchments',
    'accumulateIncrementalCatchments',
    'mergeShortReaches',
...
class River(watershed_workflow.utils.tinytree.Tree):
    ...
    def angle(self) -> float:
        ...
        if self.parent is None:
            ...
            return watershed_workflow.utils.geometry.computeAngle(self.parent.linestring, self.linestring)
    ...
    def plot(self, *args, **kwargs):
        ...
        ----------
        *args
          Positional arguments passed to watershed_workflow.plot.plot.linestringsWithCoords.
        **kwargs
          Keyword arguments passed to watershed_workflow.plot.plot.linestringsWithCoords.
        ...
        return watershed_workflow.plot.plot.linestringsWithCoords(self.df.loc[inds], *args, **kwargs)
    ...
    def explore(self, column=names.ID, m=None, marker=None, name=None, **kwargs):
        ...
        **kwargs
        ...
        if 'color' not in kwargs:
            kwargs.setdefault('cmap', watershed_workflow.plot.colors.xkcd_bolds)
    ...
    def insertCoordinate(self, i: int, xy: Tuple[float, float]) -> int:
        ...
        if watershed_workflow.utils.geometry.isClose(xy, coords[i - 1]):
            # don't insert and existing point
            return i - 1
        elif i < len(coords) and watershed_workflow.utils.geometry.isClose(xy, coords[i]):
            return i
        else:
            coords.insert(i, xy)
            self.linestring = shapely.geometry.LineString(coords)
    ...
    def _isContinuous(self, child, tol: float = _tol) -> bool:
        ...
        return watershed_workflow.utils.geometry.isClose(child.linestring.coords[-1],
    ...
    def to_file(self, filename: str, **kwargs) -> None:
        ...
        $> watershed_workflow.hydro.river.River.constructRiversByDataFrame(gpd.read_file(filename))
...
def combineSiblings(n1: River,
    ...
    if new_ls is None:
        ...
        points = [
            watershed_workflow.utils.geometry.computeMidpoint(p1.coords[0], p2.coords[0])
    ...
    for child in n1.children:
        if not watershed_workflow.utils.geometry.isClose(child.linestring.coords[-1], new_ls.coords[0]):
...
def createRivers(reaches: gpd.GeoDataFrame,
    ...
    """
    if method == 'hydroseq':
        rivers = watershed_workflow.hydro.river.River.constructRiversByHydroseq(reaches)
    elif method == 'geometry':
        rivers = watershed_workflow.hydro.river.River.constructRiversByGeometry(reaches, tol)
    elif method == 'native':
        rivers = watershed_workflow.hydro.river.River.constructRiversByDataFrame(reaches)
    else:
        raise ValueError(
...
def isClose(river1: River, river2: River, tol: float) -> bool:
    ...
    """
    return all((watershed_workflow.utils.geometry.isClose(r1.linestring, r2.linestring, tol)
