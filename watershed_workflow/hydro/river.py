...
class River(watershed_workflow.utils.tinytree.Tree):
    ...
    def angle(self) -> float:
        ...
        if self.parent is None:
            ...
            return watershed_workflow.utils.utils.computeAngle(self.parent.linestring, self.linestring)
    ...
    def insertCoordinate(self, i: int, xy: Tuple[float, float]) -> int:
        ...
        if watershed_workflow.utils.utils.isClose(xy, coords[i - 1]):
            # don't insert and existing point
            return i - 1
        elif i < len(coords) and watershed_workflow.utils.utils.isClose(xy, coords[i]):
            return i
        else:
            coords.insert(i, xy)
            self.linestring = shapely.geometry.LineString(coords)
    ...
    def _isContinuous(self, child, tol: float = _tol) -> bool:
        ...
        return watershed_workflow.utils.utils.isClose(child.linestring.coords[-1],
...
def combineSiblings(n1: River,
    ...
    if new_ls is None:
        ...
        points = [
            watershed_workflow.utils.utils.computeMidpoint(p1.coords[0], p2.coords[0])
    ...
    for child in n1.children:
        if not watershed_workflow.utils.utils.isClose(child.linestring.coords[-1], new_ls.coords[0]):
...
def isClose(river1: River, river2: River, tol: float) -> bool:
    ...
    """
    return all((watershed_workflow.utils.utils.isClose(r1.linestring, r2.linestring, tol)
