...
from watershed_workflow.utils.geometry import reverseLineString
...
def _getAngles(linestrings : List[shapely.geometry.LineString]) -> np.ndarray:
    ...
    assert all(watershed_workflow.utils.geometry.isClose(linestrings[0].coords[-1], ls.coords[-1]) for ls in linestrings)

    angles = np.array([watershed_workflow.utils.geometry.computeAngle(reverseLineString(linestrings[i]),
...
def _isInternalSharpAngle(coords : np.ndarray | shapely.geometry.LineString,
    ...
    angle = watershed_workflow.utils.geometry.computeAngle(coords[i-1] - coords[i],
                                                  coords[i+1] - coords[i])
...
def _smoothInternalSharpAngles(linestring : shapely.geometry.LineString,
    ...
    if len(coords) == 3:
        ...
        if _isInternalSharpAngle(coords, 1, min_angle):
            ...
            if watershed_workflow.utils.geometry.computeDistance(coords[0], coords[2]) \
               < max(watershed_workflow.utils.geometry.computeDistance(coords[0], coords[1]),
                     watershed_workflow.utils.geometry.computeDistance(coords[1], coords[2])):
                # just remove the extra point
                return 1, shapely.geometry.LineString([coords[0], coords[2]])
            else:
                # move the point to the triangle centroid
                new_coords[1] = watershed_workflow.utils.geometry.computeTriangleCentroid(coords[0], coords[1], coords[2])
        ...
        while i < len(coords)-1:
            if _isInternalSharpAngle(coords, i, min_angle):
                ...
                if i == 1:
                    ...
                    new_coords[i] = watershed_workflow.utils.geometry.computeMidpoint(coords[i], coords[i+1])
                elif i == len(coords) - 2:
                    # borders endpoint, cannot recenter the triangle, so instead place new point at midpoint
                    new_coords[i-1,:] = np.nan
                    new_coords[i] = watershed_workflow.utils.geometry.computeMidpoint(coords[i], coords[i-1])
                    ...
                    new_coords[i] = watershed_workflow.utils.geometry.computeTriangleCentroid(coords[i-1], coords[i], coords[i+1])
                i += 2
            else:
                i += 1
...
def _spreadAngles(linestrings : List[shapely.geometry.LineString],
    ...
    for i in range(len(linestrings)-1):
        ...
        dist = watershed_workflow.utils.geometry.computeDistance(move_ls.coords[-1], move_ls.coords[-2])
        v2 = watershed_workflow.utils.geometry.projectVectorAtAngle(v1, angles[i], dist)
...
def _getOutletLinestrings(hucs : Watershed,
    ...
    for j, spine in hucs.boundaries.items():
        for i, handle in spine.items():
            ...
            if watershed_workflow.utils.geometry.isClose(ls.coords[0], outlet):
                touches.append( (handle, reverseLineString(ls), True) )
            if watershed_workflow.utils.geometry.isClose(ls.coords[-1], outlet):
    ...
    touches = sorted(touches, key=lambda touch : watershed_workflow.utils.geometry.computeAngle(reverseLineString(reach.linestring), touch[1]))
...
def _getUpstreamLinestrings(hucs : Watershed | None,
    ...
    if hucs is not None:
        for i, ls in hucs.linestrings.items():
            # can only touch at endpoints, as guaranteed by previous simplify call
            touches_first = False
            if watershed_workflow.utils.geometry.isClose(ls.coords[0], upstream_point):
                touches_first = True
                touches.append( (i, reverseLineString(ls), True) )
            if watershed_workflow.utils.geometry.isClose(ls.coords[-1], upstream_point):
    ...
    touches = sorted(touches, key=lambda touch : watershed_workflow.utils.geometry.computeAngle(reach.linestring, touch[1]))
    # confirm we did not mess up and the 0th entry is still the downstream reach
    assert touches[0][0] is None
    return touches
    ...
    for i, (_, ls_i) in enumerate(hucs.linestrings.items()):
        for j,index in enumerate([0,-1]):
            if not done[i,j]:
                ...
                assert all(watershed_workflow.utils.geometry.isClose(touch[1].coords[-1], ls_i.coords[index]) for touch in touches)

                # sort by angle from the first
                touches = sorted(touches, key=lambda touch : watershed_workflow.utils.geometry.computeAngle(reverseLineString(touches[0][1]), touch[1]))
                yield touches
