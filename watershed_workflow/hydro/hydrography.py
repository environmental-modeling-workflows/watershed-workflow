"""Functions for manipulating combinations of River and Watershed objects"""
...
#
def findOutletsByCrossings(hucs: Watershed,
                           river: River,
                           tol: float = 10,
    ...
    for i_sub, poly in enumerate(polygons):
        ...
        clusters, cluster_centroids = watershed_workflow.utils.utils.cluster(my_crossing_centroids, tol)
    ...
    crossings_clusters_indices, crossings_clusters_centroids = \
        watershed_workflow.utils.utils.cluster(all_crossings, tol)
...
def findOutletsByElevation(hucs: Watershed, elev_raster: xarray.Dataset) -> None:
    ...
    def _findOutletsByElevation_helper(polygon, crs, elev_raster):
        ...
        mesh_points_remapped = watershed_workflow.utils.warp.warpPoints(mesh_points, crs, elev_raster.crs)
        elevs = watershed_workflow.utils.utils.valuesFromRaster(mesh_points_remapped, elev_raster)
        i = np.argmin(elevs)
...
def findOutletsByHydroseq(hucs: Watershed, river: River, tol: float = 0.0) -> None:
...
def cutAndSnapCrossings(hucs: Watershed, rivers: List[River], tol: float) -> None:
        ...
        for reach in river:
            try:
                ...
                for child in reach.children:
                    watershed_workflow.plot.plot.linestringWithCoords(child.linestring, marker='x', color='m', ax=ax)
                watershed_workflow.plot.plot.linestringWithCoords(reach.parent.linestring, marker='x', color='b', ax=ax)
...
def _cutAndSnapExteriorCrossing(hucs: Watershed, reach: River, merge_tol: float) -> None:
    ...
    for b, spine in hucs.boundaries.items():
        ...
        for s, ls_handle in spine_list:
            ...
            if watershed_workflow.utils.utils.intersects(ls, r):
                logging.info('intersection found')
                new_spine, new_reach = watershed_workflow.utils.utils.cut(ls, r)
...
def _cutAndSnapInteriorCrossing(hucs: Watershed, reach: River, merge_tol: float) -> None:
...
def snapHUCsJunctions(hucs: Watershed, rivers: List[River], tol: float) -> None:
    ...
    for ls_handle, ls in hucs.linestrings.items():
        ...
        if dists.min() < tol:
            ...
            if dists[0] < tol:
                ...
                count = 0
                while watershed_workflow.utils.utils.computeDistance(new_ls[1], new_ls[0]) < tol:
                    new_ls.pop(1)
                    count += 1
            ...
            if dists[1] < tol:
                ...
                # zap points until we are outside of the ball -- otherwise we can get weird crossings
                count = 0
                while watershed_workflow.utils.utils.computeDistance(new_ls[-2], new_ls[-1]) < tol:
                    new_ls.pop(-2)
                    count += 1
                logging.debug(f"    also removed {count} other coords")
...
def _findContainingPolygon(hucs: Watershed, linestring: shapely.geometry.LineString) -> int:
...
def snapReachEndpoints(hucs: Watershed, river: River, tol: float) -> None:
    ...
    for b, component in itertools.chain(hucs.boundaries.items(), hucs.intersections.items()):
        for s, huc_ls_handle in component.items():
            ...
            new_coord = watershed_workflow.utils.utils.findNearestPoint(reach_ls.coords[-1], huc_ls, tol)
            if new_coord is not None:
                if any(watershed_workflow.utils.utils.isClose(new_coord, c) for c in huc_ls.coords):
                ...
                while len(coords) > 2 and \
                      watershed_workflow.utils.utils.computeDistance(new_coord, coords[-2]) \
                      < watershed_workflow.utils.utils.computeDistance(new_coord, coords[-1]):
                    coords.pop(-1)
                coords[-1] = new_coord
    ...
    for reach in reaches:
        ...
        reach_linestrings = [reach_ls, ] + [
            watershed_workflow.utils.utils.reverseLineString(c.linestring) for c in reach.children
        ]
            ...
            for b, component in itertools.chain(hucs.boundaries.items(),
                ...
                for s, huc_ls_handle in component.items():
                    ...
                    def _shrinkLS(ls):
                        ...
                        coords[-1] = watershed_workflow.utils.utils.computeMidpoint(
                            coords[-2], coords[-1])
                    ...
                    if any(
                            watershed_workflow.utils.utils.intersects(huc_ls, _shrinkLS(ls))
                            for ls in reach_linestrings):
                        # find the nearest point to the endpoint if it is within tol
                        new_coord = watershed_workflow.utils.utils.findNearestPoint(
                            reach_ls.coords[0], huc_ls, tol)
                        if new_coord is not None:
                            if any(
                                    watershed_workflow.utils.utils.isClose(new_coord, c)
                            ...
                            for ls, r in zip(reach_linestrings, [reach, ] + list(reach.children)):
                                ...
                                while len(coords) > 2 and \
                                      watershed_workflow.utils.utils.computeDistance(new_coord, coords[1]) \
                                      < watershed_workflow.utils.utils.computeDistance(new_coord, coords[0]):
                                    coords.pop(0)
                                coords[0] = new_coord
                                ...
                                if r is reach:
                                    ...
                                    r.linestring = watershed_workflow.utils.utils.reverseLineString(ls)
    ...
    def isEqual(p1, p2):
        if watershed_workflow.utils.utils.isClose(p1[2].linestring.coords[p1[1]],
    ...
    for huc_ls_handle, insert_list in to_add_dict2.items():
        ...
        if not watershed_workflow.utils.utils.isClose(ls.coords[0], ls.coords[-1]):
            new_coords = [[p[2].linestring.coords[p[1]], 1] for p in insert_list]
            old_coords = [
                [c, 0] for c in ls.coords
                if not any(watershed_workflow.utils.utils.isClose(c, nc[0], tol) for nc in new_coords)
            ]
        ...
        watershed_workflow.utils.utils.logMinMaxMedianSegment(new_lss, "new_lss")
