...
class Watershed:
    ...
    def __init__(self,
        ...
        self.linestrings = _HandledCollection()  # stores linestrings
        ...
        # or intersections.
        self.boundaries = _HandledCollection()  # stores handles into linestrings
        self.intersections = _HandledCollection()  # stores handles into linestrings
            ...
            for j in range(len(shapes)):
                ...
                if watershed_workflow.utils.geometry.isEmpty(inter):
                    ...
                    ihandles = self.intersections.extend(
                        [_HandledCollection([h, ]) for h in handles])
    ...
    def to_crs(self, crs : watershed_workflow.crs.CRS):
        ...
        self.linestrings = _HandledCollection(obj_handles, tmp_df['geometry'])
        self.update()
    ...
    def plot(self, *args, **kwargs):
        ...
        return watershed_workflow.plot.plot.linestringsWithCoords(self.df.set_geometry(self.df.boundary), *args, **kwargs)
    ...
    def plotAsLinestrings(self, *args, **kwargs):
        ...
        return watershed_workflow.plot.plot.linestringsWithCoords(df, *args, **kwargs)
    ...
    def explore(self,
        ...
        if (not 'color' in style_kwds or style_kwds['color'] is None) \
           and not (column in self.df and self.df[column].dtype == 'string'):
            kwargs.setdefault('cmap', matplotlib.colors.ListedColormap(watershed_workflow.plot.colors.xkcd_muted))
            try:
                ...
                if hole.area < (abs_tol**2) or hole.area < rel_tol * part.area:
                    ...
                    try:
                        i, poly = next(
                            ...
                            if watershed_workflow.utils.geometry.isNonPointIntersection(poly, hole))
...
def partition(list_of_shapes : Sequence[shapely.geometry.Polygon],
    ...
    for i in range(len(list_of_shapes)):
        ...
        for j in range(i + 1, len(list_of_shapes)):
            ...
            try:
                if watershed_workflow.utils.geometry.isVolumetricIntersection(s1, s2):
                    s2 = s2.difference(s1)
...
def intersectAndSplit(list_of_shapes : Sequence[shapely.geometry.Polygon]) -> \
    ...
    for i, s1 in enumerate(list_of_shapes):
        for j, s2 in enumerate(list_of_shapes):
            if i != j and watershed_workflow.utils.geometry.isNonPointIntersection(s1, s2):
    ...
    for i, u in enumerate(uniques):
        if not watershed_workflow.utils.geometry.isEmpty(u):
            uniques_r[i] = u
