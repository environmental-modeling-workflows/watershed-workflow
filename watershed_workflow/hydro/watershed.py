...
class Watershed:
    ...
    def __init__(self,
        ...
        for i, u in enumerate(uniques):
            if watershed_workflow.utils.utils.isEmpty(u):
                pass
        ...
        for i in range(len(shapes)):
            for j in range(len(shapes)):
                ...
                if watershed_workflow.utils.utils.isEmpty(inter):
                    pass
                ...
                if hole.area < (abs_tol**2) or hole.area < rel_tol * part.area:
                    ...
                    try:
                        i, poly = next(
                            ...
                            if watershed_workflow.utils.utils.isNonPointIntersection(poly, hole))
...
def partition(list_of_shapes : Sequence[shapely.geometry.Polygon],
    ...
    for i in range(len(list_of_shapes)):
        ...
        for j in range(i + 1, len(list_of_shapes)):
            ...
            try:
                if watershed_workflow.utils.utils.isVolumetricIntersection(s1, s2):
                    s2 = s2.difference(s1)
...
def intersectAndSplit(list_of_shapes : Sequence[shapely.geometry.Polygon]) -> \
    ...
    for i, s1 in enumerate(list_of_shapes):
        for j, s2 in enumerate(list_of_shapes):
            if i != j and watershed_workflow.utils.utils.isNonPointIntersection(s1, s2):
    ...
    for i, u in enumerate(uniques):
        if not watershed_workflow.utils.utils.isEmpty(u):
            uniques_r[i] = u
