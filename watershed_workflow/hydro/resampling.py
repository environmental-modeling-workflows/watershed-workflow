...
__all__ = [
    'TargetLength',
    'TargetLengthFixed',
    'TargetLengthByProperty',
    'TargetLengthByCallable',
    'TargetLengthByDistance',
    'resampleRivers',
    'resampleRiver',
    'resampleWatershed',
]
...
class TargetLengthFixed(TargetLength):
    is_uniform = True
...
class TargetLengthByProperty(TargetLength):
    is_uniform = True
...
class TargetLengthByCallable(TargetLength):
    is_uniform = True
...
class TargetLengthByDistance(TargetLength):
    is_uniform = False
...
@overload
def resampleRivers(rivers : List[River], target_length : TargetLength, *, keep_points : bool = False) -> None:
    """Resamples each reach based on a functor to provide the target length."""
    ...

def resampleRivers(rivers: List[River],
                   target_length: float | TargetLength | None = None,
...
@overload
def resampleRiver(river : River, target_length : TargetLength, *, keep_points : bool = False) -> None:
    """Resamples each reach based on a functor to provide the target length."""
    ...

# could use functools singledispatch here but it seems unnecessary
def resampleRiver(river : River,
                  target_length : Optional[float | TargetLength] = None,
                  *, keep_points : bool = False) -> None:
    _resampleRiverArgs(river, target_length, keep_points)
...
def _resampleRiverArgs(river : River,
    ...
    if target_length is None:
        _resampleRiver(river, TargetLengthByProperty(), keep_points)
    elif isinstance(target_length, TargetLength):
        _resampleRiver(river, target_length, keep_points)
    else:
        try:
            _resampleRiver(river, TargetLengthFixed(float(target_length)), keep_points)
        except ValueError:
...
def _resampleRiver(river : River,
    ...
    for node in river.preOrder():
        ...
        logging.debug(f'  - resampling ls: {node.linestring.length}, {min(watershed_workflow.utils.geometry.computeSegmentLengths(node.linestring))}, {min(watershed_workflow.utils.geometry.computeSegmentLengths(ls2))}')
        node.linestring = ls2
...
#
@overload
def resampleWatershed(hucs : Watershed,
                      target_length : float,
                      *, keep_points : bool = False) -> None:
...
def resampleWatershed(hucs : Watershed,
    ...
    if shp is None:
        if isinstance(target_length, tuple):
            raise ValueError('Unrecognized compute strategy arguments provided for resampleWatershed')
        else:
            try:
                ...
                _resampleWatershed(hucs, TargetLengthFixed(target_length), keep_points)

    else:
        try:
            strat = TargetLengthByDistance(target_length, shp)
        except ValueError:
            raise ValueError('Unrecognized compute strategy arguments provided for resampleWatershed')
        else:
            _resampleWatershed(hucs, strat, keep_points)

    # recreate polygon shapes after changing all the linestrings
    hucs.update()


def _resampleWatershed(huc : Watershed,
                       computeTargetLength : TargetLength,
                       keep_points : bool = False) -> None:
...
def _resampleLineStringNonuniform(linestring : shapely.geometry.LineString,
                                  computeTargetLength : TargetLength) -> shapely.geometry.LineString:
    ...
    while s < length - 1.e-4:
        def func1(s_itr):
            ...
            midp = watershed_workflow.utils.geometry.computeMidpoint(next_coord, coords[-1])
            d = computeTargetLength(midp)
            return next_coord, d
        
        def func(s_itr):
            next_coord, d = func1(s_itr)
            ds = watershed_workflow.utils.geometry.computeDistance(next_coord, coords[-1])
            return abs(d - ds)
