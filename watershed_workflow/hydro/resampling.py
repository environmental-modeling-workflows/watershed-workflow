...
def _resampleRiver(river : River,
    ...
    for node in river.preOrder():
        ...
        logging.debug(f'  - resampling ls: {node.linestring.length}, {min(watershed_workflow.utils.utils.computeSegmentLengths(node.linestring))}, {min(watershed_workflow.utils.utils.computeSegmentLengths(ls2))}')
        node.linestring = ls2
...
def _resampleLineStringNonuniform(linestring : shapely.geometry.LineString,
    ...
    while s < length - 1.e-4:
        def func1(s_itr):
            ...
            midp = watershed_workflow.utils.utils.computeMidpoint(next_coord, coords[-1])
            d = computeTargetLength(midp)
            return next_coord, d
        
        def func(s_itr):
            next_coord, d = func1(s_itr)
            ds = watershed_workflow.utils.utils.computeDistance(next_coord, coords[-1])
            return abs(d - ds)
