...
def addSurfaceRegions(m2 : Mesh2D,
    ...
    for ind in inds:
        ...
        ls = watershed_workflow.mesh.mesh.LabeledSet(names[ind], int(ind), 'CELL', ent_ids)
        m2.labeled_sets.append(ls)


def addPolygonalRegions(m2 : Mesh2D,
                        polygons : Watershed | List[Tuple[shapely.Polygon, str]],
                        volume : bool = False,
    ...
    m2 : mesh.Mesh2D
      The mesh to label.
    polygons : Watershed | List[Tuple[shapely.Polygon, str]]
      The polygons covering each region. Either a Watershed object or
    ...
    """
    if isinstance(polygons, Watershed):
        ...
        raise ValueError("polygons must be either Watershed or List[Tuple[shapely.Polygon, str]]")
    ...
    for (p,label), part in zip(polygons, partitions):
        if len(part) > 0:
            ...
            ls = watershed_workflow.mesh.mesh.LabeledSet(str(label), int(setid), 'CELL', part)
            m2.labeled_sets.append(ls)
            ls.to_extrude = True

            # add a second region, denoting this one as the top surface of faces
            setid2 = m2.getNextAvailableLabeledSetID()
            ls2 = watershed_workflow.mesh.mesh.LabeledSet(str(label) + ' surface', setid2, 'CELL', part)
...
def addWatershedAndOutletRegions(m2 : Mesh2D,
    ...
    m2 : mesh.Mesh2D
      The mesh to label.
    hucs : iterable[shapely.Polygon] or Watershed
      Watershed polygons.
    ...
    def isInsideBall(outlet, edge):
        ...
        n2 = m2.coords[edge[1]]
        c = (n1+n2) / 2.
        close = watershed_workflow.utils.geometry.isClose(outlet, tuple(c[0:2]), outlet_width)
        return close
    ...
    for label, partition, outlet in zip(hucs.df[names.NAME],
        ...
        ls = watershed_workflow.mesh.mesh.LabeledSet(label + ' boundary',
        ...
        if outlet is not None:
            ...
            ls = watershed_workflow.mesh.mesh.LabeledSet(label + ' outlet',
                                                    m2.getNextAvailableLabeledSetID(),
                                                    'FACE', edges)
            ls.to_extrude = True
            m2.labeled_sets.append(ls)
    ...
    if exterior_outlet:
        ...
        if len(edges) == 0:
            ...
            ls2 = watershed_workflow.mesh.mesh.LabeledSet('surface domain outlet',
...
def addRiverCorridorRegions(m2 : Mesh2D,
    ...
    rivers: list(watershed_workflow.hydro.river.RiverTree)
    ...
    for label, river in zip(labels, rivers):
        ...
        if len(river_elements) > 0:
            setid2 = m2.getNextAvailableLabeledSetID()
            ls2 = watershed_workflow.mesh.mesh.LabeledSet(label + ' surface', setid2, 'CELL',
                                                     river_elements)
            m2.labeled_sets.append(ls2)
...
def addStreamOrderRegions(m2 : Mesh2D,
    ...
    for label, part in zip(labels, partitions):
        if len(part) > 0:
            setid2 = m2.getNextAvailableLabeledSetID()
            ls2 = watershed_workflow.mesh.mesh.LabeledSet(label, setid2, 'CELL', part)
            m2.labeled_sets.append(ls2)
...
def addReachIDRegions(m2 : Mesh2D,
    ...
    m2: Mesh2D
      2D mesh elevated on DEMs
    river: watershed_workflow.hydro.river.RiverTree
      List of rivers used to create the river corridors.
    reaches: list(str)
    ...
    for label, part in regions.items():
        if len(part) > 0:
            setid2 = m2.getNextAvailableLabeledSetID()
            ls2 = watershed_workflow.mesh.mesh.LabeledSet(label + ' surface', setid2, 'CELL', part)
            m2.labeled_sets.append(ls2)
...
def addDischargeRegions(m2, discharge_points, labels=None, include_cells=True, buffer_width=1):
    ...
    Parameters
    ----------
    m2 : watershed_workflow.mesh.mesh.Mesh2D
    ...
    for discharge_point, label in zip(discharge_points, labels):
        ...
        if discharge_edges:  # Only create labeled set if edges were found
            ls2 = watershed_workflow.mesh.mesh.LabeledSet(label,
                                                     m2.getNextAvailableLabeledSetID(), 'FACE', discharge_edges)
            ls2.to_extrude = True
            m2.labeled_sets.append(ls2)
            
            if include_cells and len(discharge_cells) > 0:
              ls2 = watershed_workflow.mesh.mesh.LabeledSet(label + ' cells',
...
def findDischargeEdgesCells(m2, discharge_point, include_cells=True, buffer_width=1):
    ...
    bank_node_ids = watershed_workflow.mesh.condition._findBankVerticesFromElem(m2, discharge_quad)  
    discharge_edges = [
