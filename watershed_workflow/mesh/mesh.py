...
class Mesh2D:
    ...
    watershed_workflow.mesh.triangulation.triangulate() call.
    ...
    def computeCentroid(self, c) -> np.ndarray:
        ...
        """
        return watershed_workflow.utils.geometry.computeCentroid([self.coords[v]
    ...
    def edge_centroids(self):
        """Edge centroids."""
        return {e: watershed_workflow.utils.geometry.computeMidpoint(self.coords[e[0]],
                                                           self.coords[e[1]])
                for e in self.edges}
    ...
    def writeMeshVTK(self, filename: str) -> None:
        """Writes to VTK.
        ...
        filename : str
            Output VTK filename.
        """
        import watershed_workflow.io.vtk
        assert (all(len(c) == 3 for c in self.conn))
        watershed_workflow.io.vtk.writeMeshVTK(filename, self.coords, { 'triangle': np.array(self.conn) })
    ...
    def read_VTK_Simplices(cls, filename):
        ...
        import watershed_workflow.io.vtk
        with open(filename, 'rb') as fid:
            data = watershed_workflow.io.vtk.read_buffer(fid)
        ...
        for i in range(len(dual_cells)):
            ...
            if is_boundary[i]:
                ...
                if dn_i == (up_i+1) % len(c):
                    ...
                    ax = watershed_workflow.plot.plot.get_ax(crs, fig)

                    mp = watershed_workflow.plot.plot.triangulation(mesh_points3,
                                                               mesh_tris,
                                                               crs,
                                                               ax=ax,
                                                               color='elevation',
                    ...
                    #watershed_workflow.plot.plot.hucs(shapes, crs, ax=ax, color='k', linewidth=1)
                    watershed_workflow.plot.plot.shply([shapely.geometry.LineString(cc_sorted), ],
                                                  crs,
                                                  ax=ax,
                                                  color='red',
                                                  linewidth=1)
                    watershed_workflow.plot.plot.shply([shapely.geometry.LineString(cb_sorted), ],
                                                  crs,
                                                  ax=ax,
                                                  color='blue',
                                                  linewidth=1)
...
class Mesh3D:
    ...
    def writeMeshVTK(self, filename: str) -> None:
        ...
        watershed_workflow.io.vtk.writeMeshVTK(filename, self.coords, { 'wedge': np.array(wedges) })
...
def mergeTwoMeshes(mesh1: Mesh2D, mesh2: Mesh2D) -> Mesh2D:
    ...
    for idx, coord in enumerate(mesh2.coords):
        ...
        for i, existing_coord in enumerate(combined_coords):
            if watershed_workflow.utils.geometry.isClose(coord[:2], existing_coord[:2], 1e-3):
                mapping[idx] = i
                found = True
                break
    ...
    m2_combined = watershed_workflow.mesh.mesh.Mesh2D(coords=np.array(combined_coords),
                                                 conn=combined_conn)
...
def refineTriangles(m2 : Mesh2D, to_refine : List[int]) -> Mesh2D:
    ...
    Note that each cell in to_refine must be:
...
def refineCorridorTriangles(m2 : Mesh2D,
    ...
    for c, midps in list(to_split.items()):
        if len(midps) == 2:
            ...
            spanning_edges = list(sorted(midps, key=lambda midp : watershed_workflow.utils.geometry.computeDistance(m2.coords[midp[1][0]],
                                                                                                           m2.coords[midp[1][1]])))
