...
class Mesh2D:
    ...
    def computeCentroid(self, c) -> np.ndarray:
        ...
        """
        return watershed_workflow.utils.utils.computeCentroid([self.coords[v]
...
def mergeTwoMeshes(mesh1: Mesh2D, mesh2: Mesh2D) -> Mesh2D:
    ...
    for idx, coord in enumerate(mesh2.coords):
        ...
        for i, existing_coord in enumerate(combined_coords):
            if watershed_workflow.utils.utils.isClose(coord[:2], existing_coord[:2], 1e-3):
                mapping[idx] = i
                found = True
                break
...
def refineCorridorTriangles(m2 : Mesh2D,
    ...
    for c, midps in list(to_split.items()):
        if len(midps) == 2:
            ...
            spanning_edges = list(sorted(midps, key=lambda midp : watershed_workflow.utils.utils.computeDistance(m2.coords[midp[1][0]],
                                                                                                           m2.coords[midp[1][1]])))
