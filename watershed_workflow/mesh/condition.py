...
def plotPitFilling(m2: Mesh2D,
    ...
    import watershed_workflow.plot.dynamic_colormaps
    ...
    old_z_cells = np.array([
        watershed_workflow.utils.geometry.computeCentroid([old_z_verts[v] for v in m2.conn[c]])
        for c in range(m2.num_cells)])
    ...
    scaler = watershed_workflow.plot.dynamic_colormaps.DynamicColormapScaler(output=output)
...
def conditionRiverMesh(m2 : Mesh2D,
    ...
    Parameters
    ----------
    m2: watershed_workflow.mesh.mesh.Mesh2D object
        2D mesh with 3D coordinates.
    river: watershed_workflow.hydro.river.River object
...
def smoothProfile(reach : River,
    ...
    """
    ls = reach.linestring
    s = watershed_workflow.utils.geometry.computeArclengths(ls)
