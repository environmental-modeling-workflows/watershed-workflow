"""Functions for computing photosynthetically active radiation (PAR) at a stream surface.

Wraps a Python port of the StreamLight model, which computes solar
angles, radiative transfer through riparian vegetation, and stream
shading to estimate the light energy reaching the stream surface given
channel geometry, riparian canopy properties, and incoming shortwave
radiation.
"""
import numpy as np
import copy
...
def computeStreamLight(lat,
                       ...
                       lai,
                       x_LAD=1):
    """Compute photosynthetically active radiation (PAR) reaching a stream surface.

    This is a functional wrapper around :class:`StreamLight` that
    configures, runs, and extracts results from the model in one call.
    ...
    -------
    dict
        Energy and shading response variables, keyed by name (e.g.
        ``'energy_total_surface_PAR_ppfd'``, ``'fraction_shade_veg'``,
        ``'fraction_shade_bank'``).
    """
    sl = StreamLight()
    ...
    sl.run()
    return { **sl.energy_response, **sl.shading_response }
