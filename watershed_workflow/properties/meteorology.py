"""Functions for converting meteorological forcing data to ATS input format.

Converts raw meteorological datasets -- primarily AORC, with legacy
support for DayMet -- into the variables and units expected by ATS
(e.g. specific humidity and pressure to vapor pressure, component wind
speeds to total wind speed, precipitation split into rain and snow).

Also provides utilities for allocating precipitation between rain and
snow based on air temperature, and for computing a "typical year" by
averaging a multi-year time series.
"""
...
def computeTypicalYear(dat: xr.Dataset,
    ...
    smooth_kwargs : dict, optional
      If provided, keyword arguments passed to ``ww.utils.smoothTimeSeries``
      to smooth all non-precipitation variables before computing the average
      year.  Default is no smoothing.
    ...
    # optionally smooth non-precipitation variables before averaging
    if smooth_kwargs is not None:
        din = watershed_workflow.utils.data.smoothTimeSeries(din, time_dim=time_dim, **smooth_kwargs)

    # compute the average year for non-precip fields
