"""Functions for manipulating land cover and leaf area index (LAI) data.

Computes per-land-cover-class LAI time series from MODIS LAI and land
cover rasters, and crosswalks land cover classifications (e.g. MODIS to
NLCD) so that LAI time series computed on one classification can be
applied to land cover data from another.

Also provides default property lookups and plotting utilities for LAI
time series by land cover class.
